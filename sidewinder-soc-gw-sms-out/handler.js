...
const {TwilioResponder} = require("./TwilioResponder");
...
module.exports.process = async event => {
    ...
    request.recipient.id_hash = platformService.botUserHash('phone', request.recipient, configResponse.data.address);
    const isForced = typeof request.force !== 'undefined' ? request.force : false;
    const isOptedOut = await platformService.isBotUserOptedOut('phone', request.recipient, configResponse.data.address);
    ...
    const twilioResponder = new TwilioResponder(configResponse.data);
    await twilioResponder.init();

    const response = await twilioResponder.sendMessage(request);
