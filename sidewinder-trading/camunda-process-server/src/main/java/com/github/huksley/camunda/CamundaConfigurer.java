...
public class CamundaConfigurer {
  ...
  public String createFilter(String name, int priority, String description, TaskQuery query,
      String... properties) {
    Filter existingFilter = (Filter) engine.getFilterService().createFilterQuery().filterName(name)
        .singleResult();
  ...
  public void recreateFilters(ApplicationStartedEvent ev) {
    ...
    String[] properties = env.getProperty("FILTER_TASK_GROUP_PROPS", defaultProps)
        .split("[ ]*\\,[ ]*");
    ...
    if (filters.get(filterName) == null) {
      createFilter(filterName, 30,
          env.getProperty("FILTER_TASK_ALL_DESCRIPTION",
